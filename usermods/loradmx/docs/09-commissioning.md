...
This document describes the end-to-end process for provisioning a LoRa-DMX device onto a LoRaWAN Network Server (LNS) and verifying a live downlink.

---

## Overview

The LoRa-DMX device is an **OTAA end device**. It generates its own credentials on first boot and requires manual registration on the LNS (Chirpstack, TTN, Helium, or compatible) before it can join.
...
4. Persist both to `cfg.json` under `loradmx.devEUI` and `loradmx.appKey`
...
[LoRaDMX] Register device on LNS before requesting join
```
...
### HTTP API
```bash
curl http://<device-ip>/json/info | python3 -m json.tool | grep -A5 LoRaDMX
```
...
1. Navigate to **Device Profiles** → Create profile: `LoRaDMX`
   - LoRaWAN MAC version: 1.0.3
   - Regional parameters: US915 (subband 2, channels 8-15 + 65)
   - Class: C
   - ADR: disabled
   - Codec: paste the Chirpstack codec from `LoRa-DMX/chirpstack_codec.js`
2. Navigate to **Devices** → Add Device
   - Device name: any (e.g., `loradmx-venue-east`)
   - Device Profile: `LoRaDMX`
...
```bash
# Poll device state
watch -n 2 'curl -s http://<device-ip>/json/info | python3 -c "import sys,json; d=json.load(sys.stdin); print(d[\"u\"][\"LoRaDMX\"][\"joinState\"])"'
...
```
[LoRaDMX] Sending JoinRequest...
[LoRaDMX] JoinAccept received - OTAA joined
[LoRaDMX] devAddr: 01234567
[LoRaDMX] rxWindows: open (Class C)
```
...
```
[LoRaDMX] Downlink received: FPort=1 len=1 payload=[AA]
[LoRaDMX] CMD: Test → green
[LoRaDMX] Applied to WLED: {on:true, bri:255, seg:[{fx:0, col:[[0,255,0]]}]}
```
...
Or query directly:
```bash
curl http://<device-ip>/json/info | python3 -m json.tool | grep -A20 LoRaDMX
```
...
curl -X POST http://<device-ip>/json/cfg \
  ...
  -d '{"loradmx": {"resetCredentials": true}}'
```
