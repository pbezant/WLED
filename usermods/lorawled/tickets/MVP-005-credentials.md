...
- Implement `resetCredentials` flag: POST to `/json/cfg` with `{"lorawled":{"resetCredentials":true}}` regenerates credentials and reboots
...
- [x] `pio run -e heltec_lorawled` compiles without errors
