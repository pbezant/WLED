...
|-------|-------|
| **ID** | MVP-013 |
| **Title** | PlatformIO Environment: heltec_lorawled Compile Target |
| **Role** | Coder |
| **Status** | completed |
| **Priority** | P0 |
...
Ensure `usermods/lorawled/library.json` exists with `"libArchive": false` and correct dependencies.
...
- [x] `pio project config --json-output` lists `heltec_lorawled` as an environment
- [x] All `-D` flags are present (`USERMOD_LORAWLED`, `WLED_ENABLE_DMX`, `DMX_TX_PIN=19`, all 6 `WLED_DISABLE_*`, `WLED_RELEASE_NAME`)
