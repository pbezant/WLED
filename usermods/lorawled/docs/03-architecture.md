...
|  - Device Registry        |                           |  + lorawled usermod  |
+---------------------------+                           |                     |
                                                        |  Outputs:           |
...
**The `lorawled` usermod does not modify any of these behaviors.**

### 2. `lorawled` Usermod (this project)
...
| LoRa radio | `lorawled` usermod | `SX126x-Arduino` + `LoraManager2` on SPI2 |
| Configuration | WLED core (`cfg.json`) | `addToConfig` / `readFromConfig` hooks |
| Diagnostics | `lorawled` usermod | `addToJsonInfo` / `addToJsonState` hooks |
...
|-----|-----------|-------|-------|
| SPI2 (FSPI) | SX1262 LoRa radio | SCK=9, MISO=11, MOSI=10 | `lorawled` usermod |
...
| GPIO | Function | Owner |
|------|----------|-------|
| 9    | SX1262 SCK (SPI2) | `lorawled` usermod |
| 10   | SX1262 MOSI (SPI2) | `lorawled` usermod |
| 11   | SX1262 MISO (SPI2) | `lorawled` usermod |
| 8    | SX1262 NSS (CS) | `lorawled` usermod |
| 12   | SX1262 RST | `lorawled` usermod |
| 13   | SX1262 BUSY | `lorawled` usermod |
| 14   | SX1262 DIO1 (interrupt) | `lorawled` usermod |
...
```
Device            WLED Core         lorawled Usermod     LNS
  |                  |                    |               |
  |-- power on ----> |                    |               |
  |                  |-- readFromConfig ->|               |
  |                  |        (no appKey found)           |
