...
The `lorawled` usermod injects additional fields into three WLED JSON endpoints. All fields are namespaced under `u.LoRaWLED` to prevent collisions.

---

## `/json/info` — Device Information

The usermod adds a `LoRaWLED` key to the `u` (usermod) object.
