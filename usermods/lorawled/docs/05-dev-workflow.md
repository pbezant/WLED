...
### Step 2 — Ticket
- Tickets live in `usermods/lorawled/tickets/` as individual `.md` files.
...
- Two types of tests:
  - **Compilation test:** `pio run -e heltec_lorawled` must succeed with zero errors.
...
- After every merge, run:
  ...
  pio run -e heltec_lorawled   # firmware compilation (~15-20min first run)
  ```
...
| `lorawled/dev` | Active development; all MVP tickets merge here |
| `lorawled/release` | Validated builds promoted here |

Never commit directly to `main`. All work goes through `lorawled/dev`.
...
# Build firmware for Heltec V3 with lorawled usermod
