...
This document describes everything needed to compile the `heltec_lorawled` firmware environment from scratch: PlatformIO configuration, library manifest, build flags, required source patches, and build command reference.
...
Place at `usermods/lorawled/library.json`. This file is required for PlatformIO to resolve usermod dependencies.
...
| Flag | Value | Purpose |
|------|-------|---------|
| `USERMOD_LORAWLED` | (defined) | Activates the `#ifdef` guard in `usermod_lorawled.h` and enables `REGISTER_USERMOD()` call |
...
| `WLED_RELEASE_NAME` | `"LoRa-WLED"` | Shown in `/json/info` as `ver` prefix |
...
> After applying patches: run `npm run build` then `pio run -e heltec_lorawled` to confirm compilation succeeds.
...
```cpp
// In usermod_lorawled.h or LoraDmxManager.cpp
#include <SPI.h>
...
pio run -e heltec_lorawled --target upload --upload-port <device-ip>

# 7. Clean build cache (if stale)
pio run -e heltec_lorawled --target clean
...
- [ ] `pio run -e heltec_lorawled` compiles without errors or warnings
...
- [ ] `usermods/lorawled/library.json` is present and valid JSON
- [ ] `platformio_override.ini` is present and contains `[env:heltec_lorawled]`
...
| `USERMOD_ID_LORAWLED redefined` | Conflicting ID (59 already used) | Change `USERMOD_ID_LORAWLED` to unused ID |
...
| `undefined reference to REGISTER_USERMOD` | Header not included | Include `usermod_lorawled.h` from `usermods_list.h` (if applicable) or ensure `custom_usermods = lorawled` in env |
